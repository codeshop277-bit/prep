...
# in postgress
CREATE OR REPLACE  FUNCTION employee_insert_func()
RETURNS TRIGGER 
LANGUAGE plpgsql
AS $$
BEGIN
	INSERT INTO employee_deographics(employee_id, first_name, last_name)
	VALUES (NEW.employee_id, NEW.first_name, NEW.last_name);
	RETURN NEW;
END
$$;
...
CREATE TRIGGER employee_insert
	AFTER INSERT ON employee_salary
	FOR EACH ROW
	EXECUTE FUNCTION employee_insert_func()


	Feature `			MySQL 										PostgreSQL 
	Structure			Trigger contains logic directly				Trigger calls a separate functionDelimiter	 Need to change delimiter (DELIMITER $$)		Not neededLanguageNo language specifiedMust specify LANGUAGE plpgsqlReturn statementNot requiredMust RETURN NEW or RETURN OLD
	$$;

	# Events
Postgress ql does not have events
With events we can schedule queries at a certain time period
Use pg_corn to handle events

CREATE EXTENSION pg_corn
SELECT cron.schedule(
    'delete_retirees',           -- job name
    '0 0 1 * *',                 -- cron expression (1st day of month at midnight)
